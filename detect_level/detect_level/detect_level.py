...
class DetectLevel(Node):
    def __init__(self):
        ...
        if self.sub_image_type == "compressed":
            ...
            self.sub_image_original = self.create_subscription(Image, '/image_raw', self.cbGetImage, QoSProfile(depth=1))
            self.sub_image_original
        ...
        self.sub_level_crossing_order = self.create_subscription(UInt8, '/detect/level_crossing_order',  self.cbLevelCrossingOrder, 1)
        self.sub_level_crossing_finished = self.create_subscription(UInt8, '/control/level_crossing_finished',  self.cbLevelCrossingFinished, 1)

        self.pub_level_crossing_return = self.create_publisher(UInt8, '/detect/level_crossing_stamped', 1)
        self.pub_parking_start = self.create_publisher(UInt8, '/control/level_crossing_start', 1)
    ...
    def fnFindLevel(self):
        ...
        cv2.imshow("detcet", cv_image_mask)
        cv2.waitKey(1)
